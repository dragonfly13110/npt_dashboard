...
if (!PROJECT_REF || !ACCESS_TOKEN) {
    console.error('Missing required env: SUPABASE_PROJECT_REF and SUPABASE_ACCESS_TOKEN');
    process.exit(1);
}
...
async function checkAndSchedule() {
    try {
        ...
        const result = await runSQL("SELECT MAX(updated_at) as latest_update FROM farmer_registry;");
        ...
        if (!latestUpdate || (now.getTime() - latestUpdate.getTime() >= threeDaysMs)) {
            // Need to run! Let's check target time for today (random between 6 AM and 8 AM)
            console.log('📅 Last update was more than 3 days ago (or never). Determining run window...');
            ...
            console.log(`🎯 Target run time for today: ${targetTime.toLocaleTimeString('th-TH')} (Random delay of ${randomDelayMinutes} minutes)`);

            if (now.getTime() < targetTime.getTime()) {
                // Target is in the future today
                const waitMs = targetTime.getTime() - now.getTime();
                const waitMinutes = Math.round(waitMs / 60000);
                console.log(`⏳ Waiting for ${waitMinutes} minutes until the target run window today...`);
                await delay(waitMs);
                ...
                console.log('⚡ Target run time has already passed today. Running scraper immediately to catch up.');
                await runScraper();
            }
        } else {
            ...
            console.log(`✅ Database is up to date (last updated on ${latestUpdate.toLocaleString('th-TH')})`);
            console.log(`📅 Next scheduled run: ${nextRunDate.toLocaleString('th-TH')} (in approximately ${waitHours} hours)`);
