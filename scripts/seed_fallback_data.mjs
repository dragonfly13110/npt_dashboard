...
import { toFloodDisasterRecord } from '../src/utils/floodData.js';
...
async function main() {
  ...
  const disastersOnly = process.argv.includes('--disasters-only');
  ...
  if (fs.existsSync(disastersFile)) {
    ...
    const mappedDisasters = rawDisasters.map(toFloodDisasterRecord);
