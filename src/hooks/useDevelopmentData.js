...
const countBy = (rows, key, limit = 12) => Object.entries(rows.reduce((acc, row) => {
...
export function useDevelopmentData() {
    const fetchDevelopmentData = async () => {
        const [ce, sf, ysf, career, housewife, young, fi, tourism, disasters] = await Promise.all([
            supabase.from('community_enterprises').select('*'),
            supabase.from('smart_farmer_sf').select('*'),
            supabase.from('young_smart_farmer_ysf').select('*'),
            supabase.from('agricultural_career_groups').select('*'),
            supabase.from('housewife_farmer_groups').select('*'),
            supabase.from('young_farmer_groups_detailed').select('*'),
    ...
    const { data, isLoading: loading } = useApiCache('development-dashboard-data-v2', fetchDevelopmentData);
    const {
        communityData = [],
        sfData = [],
        ysfData = [],
    ...
    const activeHousewife = useMemo(() => latestYearRows(housewifeData, 'year'), [housewifeData]);
    const activeYoung = useMemo(() => latestYearRows(youngGroupData), [youngGroupData]);
    const activeDisasters = useMemo(() => latestYearRows(disasterData, 'year'), [disasterData]);

    const ceStats = useMemo(() => ({
        total: communityData.length,
        districts: countBy(communityData, 'district', 6).map((item) => [item.name, item.value]),
    }), [communityData]);
    ...
    const groupStats = useMemo(() => {
        ...
        return {
            ...
            housewifeMembers,
            youngMembers,
            totalGroups: activeCareer.rows.length + activeHousewife.rows.length + activeYoung.rows.length,
            totalMembers: careerMembers + housewifeMembers + youngMembers,
            income: sum(activeCareer.rows, 'income') + sum(activeHousewife.rows, 'income') + sum(activeYoung.rows, 'income'),
        };
    ...
    const fiStats = useMemo(() => {
        ...
        return { total, ce, housewives, young_grp: youngGrp, career, village, sf, ysf };
    }, [farmerInstData]);

    const tourismStats = useMemo(() => ({
        total: tourismData.length,
        byType: countBy(tourismData, 'spot_type', 4).map((item) => [item.name, item.value]),
    ...
    const fiPie = useMemo(() => FI_GROUP_TYPES.map((type) => ({
        name: type.label,
        value: sum(farmerInstData, type.key),
        color: type.color,
    })).filter((item) => item.value > 0).sort((a, b) => b.value - a.value), [farmerInstData]);

    const groupComposition = useMemo(() => [
        { name: 'วิสาหกิจชุมชน', value: communityData.length, color: '#0969da' },
        { name: 'กลุ่มส่งเสริมอาชีพ', value: activeCareer.rows.length, color: '#8250df' },
        { name: 'กลุ่มแม่บ้าน', value: activeHousewife.rows.length, color: '#1a7f37' },
        { name: 'กลุ่มยุวเกษตรกร', value: activeYoung.rows.length, color: '#bf8700' },
        { name: 'ท่องเที่ยวเกษตร', value: tourismData.length, color: '#0ea5e9' },
    ].filter((item) => item.value > 0), [communityData.length, activeCareer, activeHousewife, activeYoung, tourismData.length]);

    const districtStack = useMemo(() => makeDistrictStack([
    ...
    ]), [communityData, activeCareer, activeHousewife, activeYoung, tourismData, activeDisasters]);

    const peopleDistrictStack = useMemo(() => makeDistrictStack([
