...
export function selectAgriStats({ agriAreaData, dStats }) {
  ...
  (agriAreaData || []).forEach((row) => {
    ...
    if (dStats[d]) {
      ...
      dStats[d].totalArea =
        (dStats[d].totalArea || 0) + (Number(row.total_area_rai) || 0);
      dStats[d].hort =
        (dStats[d].hort || 0) + (Number(row.horticulture_rai) || 0);
    }
  });
...
export function selectCenterCounts({ lcData, pcData, sfcData, dStats }) {
  (lcData || []).forEach((row) => {
    let d = normalizeDistrict(row.district);
    if (dStats[d]) dStats[d].lc++;
  });
  (pcData || []).forEach((row) => {
    let d = normalizeDistrict(row.district);
    if (dStats[d]) dStats[d].pc++;
  });
  (sfcData || []).forEach((row) => {
    let d = normalizeDistrict(row.district);
    if (dStats[d]) dStats[d].sfc++;
  });
  return dStats;
}
...
export function selectEnrichedStats({
  sfSfData,
  ysfData,
  disasterData,
  pestData,
