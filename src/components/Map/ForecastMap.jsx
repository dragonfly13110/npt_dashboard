...
export default function ForecastMap({ data = [] }) {
  ...
  const centerLng =
    mapPoints.reduce((sum, p) => sum + p.lng, 0) / mapPoints.length;
  ...
  return (
    <div className="forecast-map-wrapper">
              ...
              <GeoJSON
                ...
                onEachFeature={(feature, layer) => {
                  const name =
                    ...
                    feature.properties?.amp_en ||
                    feature.properties?.AMP_NAMT;
                  if (name) {
                    layer.bindTooltip(`อำเภอ${name}`, {
                      sticky: true,
                      direction: 'auto',
                    });
                  }
        ...
        {mapPoints.map((item, idx) => {
          ...
          return (
            ...
            >
              <Popup>
                <div
                  style={{ fontFamily: 'inherit', minWidth: 200, fontSize: 13 }}
                >
                  <div
                    style={{
                      fontWeight: 700,
                        ...
                        <tr>
                          ...
                          <td
                            style={{
                              fontWeight: 600,
                              color:
                                CROP_STATUS_COLORS[item.crop_status] ||
                                '#656d76',
                            }}
                          >
                            {item.crop_status}
      ...
      >
        ...
        {Object.entries(PLOT_TYPE_COLORS)
          .filter(([k]) => k !== 'ศจช')
          .map(([label, color]) => (
            <div
              key={label}
