...
export function getCropColor(cropName) {
  ...
  if (CROP_COLORS[name.replace('/', '')])
    return CROP_COLORS[name.replace('/', '')];
  ...
  if (
    name.includes('อ้อย') ||
    name.includes('ข้าวโพด') ||
    name.includes('มันสำปะหลัง') ||
    name.includes('พืชไร่')
  ) {
  ...
  if (
    name.includes('ผัก') ||
    name.includes('คะน้า') ||
    name.includes('กุยช่าย') ||
    name.includes('ถั่วฝักยาว') ||
    name.includes('บัวบก') ||
    name.includes('มะเขือ') ||
    name.includes('หน่อไม้ฝรั่ง')
  ) {
  ...
  if (
    name.includes('ฝรั่ง') ||
    name.includes('มะม่วง') ||
    name.includes('ส้มโอ') ||
    name.includes('กล้วย') ||
    name.includes('มะพร้าว') ||
    name.includes('อินทผาลัม') ||
    name.includes('ชมพู่') ||
    name.includes('ลำไย') ||
    name.includes('ไม้ผล') ||
    name.includes('ไม้ยืนต้น')
  ) {
    return CROP_COLORS['ไม้ผลไม้ยืนต้น']; // #047857
  }
  if (
    name.includes('ดอก') ||
    name.includes('ประดับ') ||
    name.includes('กล้วยไม้') ||
    name.includes('ไทร')
  ) {
    return CROP_COLORS['ไม้ดอกไม้ประดับ']; // #ec4899
  }
  if (
    name.includes('สมุนไพร') ||
    name.includes('เครื่องเทศ') ||
    name.includes('ขิง') ||
    name.includes('ข่า')
  ) {
...
export function barOption(
  ...
  const normalizedSeries = series.map((entry, index) => {
    ...
    return {
      ...
      itemStyle: {
        borderRadius: layout === 'vertical' ? [0, 6, 6, 0] : [6, 6, 0, 0],
      },
    };
  });
  ...
  const valueAxis = {
    ...
    axisLabel: {
      formatter: (value) =>
        compact
          ? formatCompactValue(value, digits)
...
export function comboOption(data, series, options = {}) {
  const base = barOption(
    data,
    series.map((entry) => ({ ...entry, type: undefined })),
    options
  );
  return {
    ...base,
    series: series.map((entry, index) => {
      if (entry.type === 'line') {
        return {
          ...
          lineStyle: {
            color:
              entry.color || options.colors?.[index] || FALLBACK_COLORS[index],
            width: 3,
          },
          itemStyle: {
            color:
...
export function radarOption(
  ...
  const maxValue =
    max || Math.max(1, ...data.map((item) => Number(item[valueKey]) || 0));
  return {
    tooltip: {
      trigger: 'item',
      formatter: (item) => {
        const rows = data
          .map(
            (row, index) =>
              `<div style="margin-top:4px;color:#334155">${row[labelKey]}: <b>${formatValue(item.value[index], 0)}</b></div>`
          )
          .join('');
...
export function treemapOption(
  data,
  { colors = FALLBACK_COLORS, valueKey = 'size', unit = '' } = {}
) {
  return {
    tooltip: {
      trigger: 'item',
      formatter: (item) =>
        `${htmlTooltipTitle(item.name)}<div style="color:#334155">${dot(item.color)}จำนวน: <b>${formatValue(item.value, 0)}${unit ? ` ${unit}` : ''}</b></div>`,
    },
    series: [
      {
        type: 'treemap',
...
function axisTooltip({ unit = '', digits = 0, totalKey } = {}) {
  return {
    ...
    axisPointer: {
      type: 'shadow',
      shadowStyle: { color: 'rgba(15,23,42,0.045)' },
    },
    formatter: (params) => {
      ...
      const lines = rows
        ...
        .map(
          (item) =>
            `${dot(item.color)}${item.seriesName}: <b>${formatValue(item.value, digits)}${unit ? ` ${unit}` : ''}</b>`
        )
        .map(
          (line) => `<div style="margin-top:4px;color:#334155">${line}</div>`
        )
        .join('');
      const totalLine =
        totalKey && source[totalKey] != null
...
export function dualAxisOption(
  data,
  series,
  { categoryKey = 'name', rotate = 0, grid = {}, legend = true } = {}
) {
  ...
  return {
    tooltip: {
      ...
      formatter: (params) => {
        ...
        const lines = rows
          .map(
            (item) =>
              `${dot(item.color)}${item.seriesName}: <b>${formatValue(item.value, 1)} ${item.seriesIndex === 0 ? 'mm' : '°C'}</b>`
          )
          .map(
            (line) => `<div style="margin-top:4px;color:#334155">${line}</div>`
          )
    ...
    grid: {
      ...compactGrid,
      bottom: legend ? 48 : compactGrid.bottom,
      right: 48,
      ...grid,
    },
    xAxis: {
    ...
    yAxis: [
      {
        ...
        axisLabel: {
          formatter: (value) => `${formatValue(value, 0)} mm`,
          color: '#667085',
          fontSize: 11,
        },
        splitLine: { lineStyle: { color: '#eef2f7', type: 'dashed' } },
      ...
      },
      {
        ...
        nameTextStyle: { color: '#667085', fontSize: 11 },
        axisLabel: {
          formatter: (value) => `${formatValue(value, 0)} °C`,
          color: '#667085',
          fontSize: 11,
        },
