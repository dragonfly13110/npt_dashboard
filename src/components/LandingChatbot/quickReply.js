...
const NAVIGATION_REPLIES = [
  [
    ...
    'ดูได้ที่ [แผนที่อัจฉริยะ](/smart-map) ค่ะ',
  ],
  [
    /^(ขอ)?(ดู|เปิด|ไป|ไปดู)?\s*ราคาสินค้าเกษตร$/i,
    'ดูได้ที่ [ราคาสินค้าเกษตร](/public/agricultural-prices) ค่ะ',
  ],
  [
    /^(ขอ)?(ดู|เปิด)?\s*พยากรณ์โรค(พืช)?$/i,
    'ดูได้ที่ [พยากรณ์โรคและแมลง](/public/disease-forecast) ค่ะ',
  ],
  [
    /^(ขอ)?(ดู|เปิด)?\s*คู่มือ(การใช้งาน|ใช้งาน)?$/i,
...
export function getLandingQuickReply(text) {
  ...
  if (GREETING_RE.test(query) || HELP_RE.test(query)) {
    return 'สวัสดีค่ะ หนูคือน้องข้าวหลาม ถามเรื่องข้อมูลเกษตรนครปฐม แผนที่ ราคา พื้นที่เพาะปลูก โรคพืช หรือเมนูในระบบได้เลยค่ะ';
  }
