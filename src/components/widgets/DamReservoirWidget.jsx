...
function getStorageInfo(pct) {
    if (pct >= 80) return { level: 'สูง', color: '#3b82f6', bg: '#dbeafe', emoji: '🔵', tip: 'น้ำเพียงพอ เหมาะสำหรับการเพาะปลูกทุกชนิด' };
    if (pct >= 50) return { level: 'ปานกลาง', color: '#10b981', bg: '#d1fae5', emoji: '🟢', tip: 'น้ำพอเพียง วางแผนการใช้น้ำอย่างเหมาะสม' };
    if (pct >= 30) return { level: 'น้อย', color: '#f59e0b', bg: '#fef3c7', emoji: '🟡', tip: 'น้ำเริ่มน้อย ควรประหยัดน้ำและเลือกพืชที่ใช้น้ำน้อย' };
    return { level: 'วิกฤต', color: '#ef4444', bg: '#fee2e2', emoji: '🔴', tip: 'น้ำน้อยมาก! งดทำนาปรัง หันมาปลูกพืชใช้น้ำน้อย' };
}
...
async function fetchDamData() {
    ...
    const allDams = json.data.flatMap(region => region.dam.map(d => ({ ...d, region: region.region })));
...
export default function DamReservoirWidget({ defaultExpanded = false }) {
    ...
    return (
        <div className="widget-box slide-up-anim" style={{
            animationDelay: '0.4s', display: 'flex', flexDirection: 'column', gap: 14, padding: 20,
            ...
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                        <h4 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: '#0f172a' }}>สถานการณ์น้ำ — ผลกระทบต่อ จ.นครปฐม</h4>
            ...
            <div style={{
                ...
                background: overallInfo.bg, padding: '12px 16px', borderRadius: 12,
                border: `1px solid ${overallInfo.color}25`
            }}>
                <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#64748b' }}>แหล่งน้ำต้นทุนหลัก (ศรีนครินทร์ + วชิราลงกรณ)</div>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 4 }}>
                        <span style={{ fontSize: 22, fontWeight: 900, color: '#0f172a' }}>{data.combinedVolume.toLocaleString()}</span>
                        <span style={{ fontSize: 12, color: '#64748b', fontWeight: 600 }}>/ {data.combinedCapacity.toLocaleString()} ล้าน ม³</span>
                    </div>
                </div>
    ...
    return (
        <div style={{ position: 'relative', width: 68, height: 68, flexShrink: 0 }}>
            ...
            <div style={{
                ...
                <span style={{ fontSize: 15, fontWeight: 900, color, lineHeight: 1 }}>{percent.toFixed(0)}%</span>
                <span style={{ fontSize: 9, color: '#94a3b8', fontWeight: 600 }}>รวม</span>
            </div>
        </div>
    );
}
