  ...
  },
  {
    ...
    zones: [
      ...
      },
      {
        ...
        emoji: '💧',
        points: [
          { lat: 14.03, lon: 99.991 },
          { lat: 14.01, lon: 100.011 },
        ],
      },
    ],
  },
...
function getMoistureInfo(val) {
  if (val <= 0.15)
    return {
      ...
      color: '#ef4444',
      emoji: '🏜️',
      tip: 'ดินแห้งจัด ต้องให้น้ำทันที เสี่ยงพืชเหี่ยวเฉา',
    };
  if (val <= 0.22)
    return {
      level: 'แห้ง',
      color: '#f59e0b',
      emoji: '🌤️',
      tip: 'ดินค่อนข้างแห้ง ควรให้น้ำเพิ่มสำหรับพืชใบ/พืชผัก',
    };
  if (val <= 0.32)
    return {
      level: 'เหมาะสม',
      color: '#10b981',
      emoji: '✅',
      tip: 'ดินชุ่มชื้นพอเหมาะ เหมาะกับการเพาะปลูกทุกชนิด',
    };
  if (val <= 0.4)
    return {
      level: 'ชุ่มชื้นดี',
      color: '#3b82f6',
      emoji: '💧',
      tip: 'ดินมีน้ำอุดมสมบูรณ์ ลดการให้น้ำลงได้',
    };
  return {
    level: 'อิ่มน้ำ',
    color: '#8b5cf6',
    emoji: '🌊',
    tip: 'ดินอิ่มตัวด้วยน้ำ ระวังน้ำท่วมขัง หลีกเลี่ยงการไถพรวน',
  };
}
...
function renderLine(history, key, color, spacing) {
  ...
  return (
    <>
      <polygon
        points={`0,${chartHeight} ${points} ${(history.length - 1) * spacing},${chartHeight}`}
        fill={color}
        opacity="0.1"
      />
      <polyline
...
export default function SoilMoistureWidget({
  ...
  const fetchFn = useCallback(async () => {
    try {
      const zones = await Promise.all(
        district.zones.map(async (zone) => {
          const pointData = await Promise.all(
            zone.points.map(fetchPointSoilData)
          );
          const aggregated = aggregateZoneData(zone.label, pointData);
          return aggregated
            ? { ...aggregated, key: zone.key, emoji: zone.emoji }
            : null;
        })
      );
      ...
      const mockZones = district.zones.map((z) => ({
        ...
        current: {
          ...mockCurrent,
          soil_moisture_0_to_1cm: 0.25 + Math.random() * 0.1,
        },
      }));
      ...
      return {
        district: {
          ...
          pointCount: district.zones.reduce(
            (sum, z) => sum + z.points.length,
            0
          ),
        },
        zones: mockZones,
  ...
  const { data, isLoading } = useApiCache(cacheKey, fetchFn, {
    staleMinutes: 30,
    cacheMinutes: 120,
  });

  if (summary) {
    const moisture = data?.district?.current?.soil_moisture_0_to_1cm;
    const info = moisture != null ? getMoistureInfo(moisture) : null;
    return (
      <button
        className="live-kpi-card live-kpi-card--soil"
        type="button"
        onClick={onOpen}
      ...
      >
        ...
        <span className="live-kpi-copy">
          <small>ความชื้นดิน</small>
          <strong>
            {moisture != null ? `${(moisture * 100).toFixed(1)}%` : '—'}
          </strong>
          <span>
            {info ? `${info.level} · อ.${district.name}` : 'กำลังอัปเดตข้อมูล'}
          </span>
        </span>
    ...
    return (
      <div className="widget-box skeleton-pulse">
        <div className="w-loader">
          กำลังโหลดข้อมูลดินแบบหลายจุด อ.{district.name}...
        </div>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <p style={{ color: '#b91c1c' }}>
          ไม่สามารถเชื่อมต่อข้อมูลจากเซนเซอร์ภาคพื้นดินได้
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              ...
              >
                {surfaceInfo.emoji} {surfaceInfo.level}
              </span>
            </div>
            <span style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>
              📍 อ.{district.name} •{' '}
              {new Date(current.time).toLocaleTimeString('th-TH', {
                hour: '2-digit',
                minute: '2-digit',
              })}{' '}
              น. • {districtAverage.pointCount} จุดตัวแทน •{' '}
              {districtAverage.zoneCount} โซนเกษตร
            </span>
          </div>
        </div>
        <span
      ...
      {isExpanded && (
        ...
        >
          ค่าใน widget นี้มาจากการเฉลี่ยหลายพิกัดตัวแทน แยกตามโซนเกษตร เช่น นา
          สวน และพื้นที่ลุ่ม
          เพื่อให้ใกล้เคียงสภาพจริงมากกว่าการใช้จุดเดียวต่ออำเภอ
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              >
                <div>
                  <div
                    style={{ fontSize: 13, fontWeight: 800, color: '#14532d' }}
                  >
                    {zone.emoji} {zone.zoneName}
                  </div>
                  <div style={{ fontSize: 10, color: '#64748b', marginTop: 2 }}>
                    {zone.pointCount} จุดเฉลี่ย
                  </div>
                </div>
                <div
                  style={{
        ...
        <>
                ...
                <div key={depth.key}>
                  ...
                  >
                    ...
                    <span style={{ color: info.color }}>
                      {formatPercent(value)}{' '}
                      <small style={{ fontWeight: 500 }}>m³/m³</small>
                    </span>
                  </div>
                  <div
                    style={{
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: 14, fontWeight: 900, color: '#1e293b' }}>
                {current.soil_temp_surface?.toFixed(1) ?? '–'}°C
              </span>
            </div>
            ...
            >
              ...
              <span style={{ fontSize: 14, fontWeight: 900, color: '#1e293b' }}>
                {current.soil_temp_deep?.toFixed(1) ?? '–'}°C
              </span>
            </div>
          </div>
          ...
          {showHistory && history.length > 0 && (
            ...
            >
              ...
              <div
                style={{ position: 'relative', height: 120, marginLeft: 28 }}
              >
                <div
                  style={{
                    position: 'absolute',
                ...
                <svg
                  viewBox={`0 0 ${(history.length - 1) * 14} 120`}
              ...
              >
                {history
                  .filter(
                    (_, index) =>
                      index % Math.max(1, Math.floor(history.length / 5)) === 0
                  )
                  .map((entry, index) => (
          ...
          <div
            style={{
              fontSize: 11,
              background: `${surfaceInfo.color}15`,
              color: surfaceInfo.color,
              padding: '8px 12px',
              borderRadius: 8,
              fontWeight: 700,
              border: `1px dashed ${surfaceInfo.color}40`,
            }}
          >
            💡 {surfaceInfo.tip}
          </div>
