  ...
  return items
    .map((item) => {
      ...
      return {
        geometry: {
          coordinates: [
            item.geometry?.coordinates?.[0] || props.longitude,
            item.geometry?.coordinates?.[1] || props.latitude,
          ],
        },
        properties: {
          ...props,
          brightness: parseFloat(
            props.bright_ti4 || props.bright_ti5 || props.brightness || 0
          ),
        },
      };
    })
...
function getMockHotspots(dayRange) {
  ...
  return Array.from({ length: n }, (_, i) => ({
    ...
    properties: {
      ...
      ap_tn: districts[i % districts.length],
      tb_tn: 'ตำบลตัวอย่าง',
      satellite: satellites[i % satellites.length],
      lu_name: landuses[i % landuses.length],
...
function toThaiTime(thDate, thTime, acqDate, acqTime) {
  try {
    if (thDate) {
      ...
      const t = thTime
        ? `${String(thTime).padStart(4, '0').slice(0, 2)}:${String(thTime).padStart(4, '0').slice(2)}`
        : '';
    ...
    if (acqDate) {
      ...
      const d = new Date(
        `${dateOnly}T${hhmm.slice(0, 2)}:${hhmm.slice(2)}:00Z`
      );
...
const LANDUSE_COLORS = {
  พื้นที่เกษตร: '#dc2626',
  'ชุมชนและอื่น ๆ': '#f59e0b',
  พื้นที่ริมทางหลวง: '#6366f1',
  'เขต สปก.': '#10b981',
  ป่าสงวน: '#059669',
};
const DAY_OPTIONS = [
...
function normalizeSatelliteName(value) {
  ...
  if (
    ...
    compact === 'npp' ||
    compact === 'snpp' ||
    compact.includes('suomi')
  )
    return 'Suomi NPP';
  if (
    compact === 'n20' ||
    compact.includes('noaa20') ||
    compact.includes('jpss1')
  )
    return 'NOAA-20';
  if (
    compact === 'n21' ||
    compact.includes('noaa21') ||
    compact.includes('jpss2')
  )
    return 'NOAA-21';
  return raw;
}
...
export default function HotspotWidget({
  ...
  const mockFeatures = useMemo(
    () => (useMock ? getMockHotspots(fetchDayRange) : []),
    [useMock, fetchDayRange]
  );
  const localHotspots = useMemo(() => {
    ...
    return [...raw].sort((a, b) => {
      ...
      return `${dateB}T${String(timeB).padStart(4, '0')}`.localeCompare(
        `${dateA}T${String(timeA).padStart(4, '0')}`
      );
    });
  ...
  const rangedHotspots = useMemo(() => {
    ...
    return localHotspots.filter((f) =>
      isInIsoDateRange(
        getIsoDate(f.properties),
        customDateRange.start,
        customDateRange.end
      )
    );
  ...
  const satelliteHotspots = useMemo(() => {
    ...
    return rangedHotspots.filter(
      (f) => getSatelliteName(f.properties) === selectedSatellite
    );
  }, [rangedHotspots, selectedSatellite]);

  const amphoeStats = useMemo(() => {
    const m = {};
    satelliteHotspots.forEach((f) => {
      const n = f.properties?.ap_tn || 'ไม่ทราบ';
      m[n] = (m[n] || 0) + 1;
    });
  ...
  const landuseStats = useMemo(() => {
    const m = {};
    satelliteHotspots.forEach((f) => {
      const n = f.properties?.lu_name || 'อื่น ๆ';
      m[n] = (m[n] || 0) + 1;
    });
    return Object.entries(m).sort((a, b) => b[1] - a[1]);
  }, [satelliteHotspots]);
  ...
  const tambonStats = useMemo(() => {
    ...
    satelliteHotspots
      ...
      .forEach((f) => {
        const n = f.properties?.tb_tn || 'ไม่ทราบตำบล';
        m[n] = (m[n] || 0) + 1;
      });
    return Object.entries(m).sort((a, b) => b[1] - a[1]);
  }, [satelliteHotspots, selectedAmphoe]);

  const subdistrictHotspotCounts = useMemo(() => {
    return countRowsBySubdistrict(
      satelliteHotspots.map((f) => ({
  ...
  const visibleSubdistrictBoundaries = useMemo(() => {
    if (!selectedAmphoe) return [];
    return getSubdistrictsForDistrict(subdistrictGeoJSON, selectedAmphoe);
  }, [selectedAmphoe]);

  const filteredHotspots = useMemo(() => {
    return satelliteHotspots.filter((f) => {
      const p = f.properties || {};
      if (selectedAmphoe && (p.ap_tn || 'ไม่ทราบ') !== selectedAmphoe)
        return false;
      if (selectedTambon && (p.tb_tn || 'ไม่ทราบตำบล') !== selectedTambon)
        return false;
      return true;
    });
  ...
  const { MapContainer, TileLayer, CircleMarker, Tooltip, GeoJSON } =
    MapComponents || {};
  ...
  const handleExportCsv = async () => {
    ...
    const satellitePart =
      selectedSatellite === 'all'
        ? 'all-satellite'
        : selectedSatellite.replace(/\s+/g, '-').toLowerCase();
    const placePart =
      [selectedAmphoe, selectedTambon].filter(Boolean).join('-') || 'all-area';
    downloadCsv(
      `hotspots-npt-${dayRange}days-${satellitePart}-${placePart}-${dateStamp}.csv`,
      csv
    );
  };
  ...
  return (
    <div
      className="widget-box hotspot-widget-card slide-up-anim is-compact-layout"
      style={{ animationDelay: '0.25s', padding: 0, overflow: 'hidden' }}
    >
      {/* ═══════ TOP SECTION — full width ═══════ */}

      {/* Header */}
      <div
        className="hotspot-widget-head"
        style={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            >
              จุดความร้อน จ.นครปฐม (ข้อมูลจาก GISTDA
              ยังไม่ได้รับการยืนยันในพื้นที่)
            </h4>
            <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 400 }}>
              VIIRS / GISTDA Satellite
            </div>
          </div>
        </div>
        <div
          className="hotspot-head-actions"
          style={{
            display: 'flex',
            gap: 8,
            alignItems: 'center',
        ...
        >
          <div
            className="hotspot-widget-controls"
            style={{
              display: 'flex',
              gap: 6,
              alignItems: 'center',
          ...
          >
            ...
            {DAY_OPTIONS.map((o) => (
              <button
                ...
                style={{
                  ...
                  fontFamily: 'inherit',
                  border:
                    !hasCustomDateRange && dayRange === o.value
                      ? 'none'
                      : '1px solid #e2e8f0',
                  background:
                    !hasCustomDateRange && dayRange === o.value
            ...
            <button
              ...
              style={{
                padding: '4px 10px',
                borderRadius: 8,
                cursor: hotspotsForList.length ? 'pointer' : 'not-allowed',
                fontFamily: 'inherit',
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="hotspot-content-shell">
            ...
            <div
              className="hotspot-summary-panel"
              style={{
                padding: '12px 18px',
                borderBottom: '1px solid #f1f5f9',
                background: hasHotspots
            ...
            >
              ...
              <div
                className="hotspot-summary-row"
                style={{
                  display: 'flex',
                  gap: 16,
                  alignItems: 'flex-start',
                  marginBottom: landuseStats.length > 0 ? 10 : 0,
                }}
              >
                {/* Count */}
                <div
                  ...
                  <div>
                    <div
                      style={{
                        fontSize: 28,
                        fontWeight: 800,
                        color: hasHotspots ? '#dc2626' : '#059669',
                        lineHeight: 1,
                      }}
                    >
                      {satelliteHotspots.length}{' '}
                      <span style={{ fontSize: 13, fontWeight: 600 }}>จุด</span>
                    </div>
                    <div
                      style={{
                        fontSize: 10,
                ...
                {amphoeStats.length > 0 && (
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      กดที่อำเภอ เพื่อกลั่นกรองตำบล
                    </div>
                    <div
                      className="hotspot-amphoe-grid"
                      style={{
                        display: 'grid',
                        gridTemplateColumns:
                        ...
                        {tambonStats.map(([name, count]) => {
                          ...
                          return (
                            <button
                              ...
                              className="hotspot-hover-float"
                              onClick={() =>
                                setSelectedTambon(sel ? null : name)
                              }
                              style={{
                                padding: '3px 8px',
                                borderRadius: 12,
            ...
            <div
              id="hotspot-detail-content"
              className="hotspot-mobile-layout"
              style={{ display: 'flex', minHeight: 450 }}
            >
              {/* BOTTOM LEFT — Map */}
              <div
                className="hotspot-map-panel"
                style={{
                  flex: '1 1 55%',
                  minWidth: 0,
                  position: 'relative',
              ...
              >
                {MapComponents ? (
                  ...
                  >
                    ...
                    {geoJSONData && (
                      <GeoJSON
                        key={`geo-${selectedSatellite}-${selectedAmphoe || 'all'}-${selectedTambon || 'all'}-${customDateRange.start || 'all'}-${customDateRange.end || dayRange}`}
                        data={geoJSONData}
                        style={(feature) => {
                          const hl =
                            selectedAmphoe &&
                            feature.properties?.amp_th === selectedAmphoe;
                          return {
                        ...
                        onEachFeature={(feature, layer) => {
                          ...
                          if (!name) return;
                          const cnt =
                            amphoeStats.find(([n]) => n === name)?.[1] || 0;
                          layer.bindTooltip(
                            `<b>อ.${name}</b>${cnt > 0 ? `<br/>🔥 ${cnt} จุด` : '<br/>✅ ปลอดภัย'}`,
                    ...
                    {visibleSubdistrictBoundaries.length > 0 && (
                      <GeoJSON
                        key={`tambon-boundary-${selectedAmphoe}-${selectedTambon || 'all'}-${Object.keys(subdistrictHotspotCounts).length}`}
                        data={{
                          type: 'FeatureCollection',
                          features: visibleSubdistrictBoundaries,
                        }}
                        style={(feature) => {
                          const props = feature.properties || {};
                          const isSelected = selectedTambon === props.tam_th;
                          const hasHotspot =
                            (subdistrictHotspotCounts[props.tam_code] || 0) > 0;
                          return {
                            color: isSelected
                              ? '#7f1d1d'
                              : hasHotspot
                        ...
                        onEachFeature={(feature, layer) => {
                          const props = feature.properties || {};
                          const name =
                            props.tam_th || props.tam_en || props.tam_code;
                          const count =
                            subdistrictHotspotCounts[props.tam_code] || 0;
                          layer.bindTooltip(
                            `<b>ต.${name}</b><br/>${count > 0 ? `🔥 ${count} จุด` : 'ไม่มีจุดความร้อน'}`,
                            { sticky: true, direction: 'auto' }
                          );
                          layer.on({
                            click: () => {
                              setSelectedAmphoe(props.amp_th || selectedAmphoe);
                              setSelectedTambon((p) =>
                                p === name ? null : name
                              );
                            },
                            ...
                            mouseout: (e) => {
                              const isSelected = selectedTambon === name;
                              const hasHotspot = count > 0;
                              e.target.setStyle({
                                fillOpacity: isSelected
                                  ? 0.35
                                  : hasHotspot
                        ...
                        >
                          <Tooltip sticky direction="top">
                            ...
                            >
                              ...
                              {p.village && (
                                <div>
                                  <b>หมู่บ้าน:</b> {p.village}
                                </div>
                              )}
                              <div>
                                <b>ประเภท:</b> {p.lu_name || '-'}
                                {p.lu_hp_name ? ` (${p.lu_hp_name})` : ''}
                              </div>
                              {thaiTime && (
                                <div>
                                  <b>เวลา:</b> {thaiTime} น.
                              ...
                              <div>
                                <b>ความร้อน:</b>{' '}
                                {p.brightness
                                  ? `${Number(p.brightness).toFixed(1)} K`
                                  : '-'}
                              </div>
                            </div>
                          </Tooltip>
              ...
              <div
                className="hotspot-detail-panel"
                style={{
                  flex: '1 1 45%',
                  minWidth: 0,
                  overflowY: 'auto',
                ...
                {filteredHotspots.length > 0 ? (
                  <div style={{ padding: '10px 14px' }}>
                    ...
                    >
                      ...
                      >
                        🔥 รายละเอียด{' '}
                        {selectedAmphoe ? `อ.${selectedAmphoe}` : ''}
                        {selectedTambon ? ` ต.${selectedTambon}` : ''} (
                        {hotspotsForList.length} จุด)
                      </div>
                      <div
                        style={{
                          fontSize: 10,
                    ...
                    {uniqueDates.length > 1 && (
                      <div style={{ marginBottom: 12 }}>
                        <select
                          ...
                          onChange={(e) =>
                            setSelectedDateFilter(e.target.value || null)
                          }
                          style={{
                            width: '100%',
                            padding: '6px 10px',
                    ...
                    {hotspotsForList.length > 0 ? (
                      <>
                        {hotspotsForList.slice(0, displayLimit).map((f, i) => {
                          ...
                          return (
                            <a
                              ...
                              href={
                                p.linkgmap ||
                                `https://maps.google.com/maps?q=${f.geometry.coordinates[1]},${f.geometry.coordinates[0]}`
                              }
                              target="_blank"
                              ...
                              style={{
                                ...
                                textDecoration: 'none',
                                color: 'inherit',
                                border: '1px solid transparent', // for hover effect compatibility
                              }}
                            >
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div
                        ...
                        {hotspotsForList.length > displayLimit && (
                          ...
                          >
                            ➕ โหลดเพิ่มอีก (
                            {hotspotsForList.length - displayLimit} จุด)
                          </button>
                        )}
                      </>
                    ) : (
