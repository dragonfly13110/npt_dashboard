...
export default function AirQualityWidget() {
        ...
        const loadAqiData = () => {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (position) => fetchDataForLoc(position.coords.latitude, position.coords.longitude),
                    (err) => {
                        console.warn("Geolocation Error/Denied, fallback to default", err);
                        fetchDataForLoc(13.82, 100.06); // Nakhon Pathom
                    },
                    { timeout: 6000 }
                );
            } else {
                fetchDataForLoc(13.82, 100.06);
            }
        };

        const fetchDataForLoc = async (lat, lon) => {
            try {
                // 1. Get Location Name
                let locName = 'นครปฐม';
                try {
    ...
    if (loading) return <div className="widget-box skeleton-pulse"><div className="w-loader">กำลังตรวจสอบพิกัดและคุณภาพอากาศ...</div></div>;
    if (!aqiData) return null;

    const info = getAqiInfo(aqiData.pm25);
    const percentage = Math.min((aqiData.pm25 / 150) * 100, 100);
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <span style={{ fontSize: '11px', color: '#64748b', fontWeight: '500' }}>
                            📍 จ.{aqiData.locationName} &bull; อัปเดต: {aqiData.timeLabel}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <div style={{ position: 'relative', width: '90px', height: '90px', flexShrink: 0 }}>
                    ...
                    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                        <span style={{ fontSize: '20px', fontWeight: '900', color: info.color, lineHeight: '1' }}>{Math.round(aqiData.pm25)}</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', marginTop: '10px', padding: '6px 10px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #f1f5f9' }}>
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <span style={{ fontSize: '13px', fontWeight: '800', color: '#334155' }}>{Math.round(aqiData.pm10)} <small style={{fontSize: '9px', fontWeight: 'normal'}}>µg/m³</small></span>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <span style={{ fontSize: '13px', fontWeight: '800', color: '#334155' }}>{Math.round(aqiData.aqi)} <small style={{fontSize: '9px', fontWeight: 'normal'}}>Index</small></span>
                        </div>
                    </div>
                </div>
            </div>
