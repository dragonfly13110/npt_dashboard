...
function getAqiInfo(pm25) {
  if (pm25 <= 15)
    return {
      ...
      color: '#10b981',
      bg: '#d1fae5',
      desc: 'คุณภาพอากาศดีมาก เหมาะสำหรับการทำกิจกรรมหรือเพาะปลูกในที่โล่ง',
      alert: '💚',
    };
  if (pm25 <= 35)
    return {
      ...
      color: '#f59e0b',
      bg: '#fef3c7',
      desc: 'คุณภาพอากาศปานกลาง ประชาชนสามารถทำงานกลางแจ้งได้ตามปกติ',
      alert: '💛',
    };
  if (pm25 <= 55)
    return {
      ...
      color: '#f97316',
      bg: '#ffedd5',
      desc: 'เกษตรกรหรือผู้ที่มีความอ่อนไหวควรดูแลตัวเอง สวมหน้ากากถ้าจำเป็น',
      alert: '🧡',
    };
  if (pm25 <= 150)
    return {
      ...
      color: '#ef4444',
      bg: '#fee2e2',
      desc: 'ควรสวมหน้ากากอนามัย N95 และงดการเผาเศษวัสดุทางการเกษตรเด็ดขาด',
      alert: '❤️',
    };
  return {
    level: 'อันตราย',
    color: '#8b5cf6',
    bg: '#ede9fe',
    desc: 'งดกิจกรรมกลางแจ้งเด็ดขาด! สภาพอากาศอันตรายต่อสุขภาพมาก',
    alert: '💜',
  };
}
...
async function fetchAqiForLocation(lat, lon) {
  ...
  try {
    ...
    if (geoJson.principalSubdivision) {
      locName = geoJson.principalSubdivision
        .replace('จังหวัด', '')
        .replace('Province', '')
        .trim();
  ...
  const res = await fetch(
    `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${lat}&longitude=${lon}&current=european_aqi,pm2_5,pm10&hourly=pm2_5&timezone=Asia%2FBangkok`
  );
...
export default function AirQualityWidget({ mini }) {
  ...
  if (mini) {
    ...
    const hourlyForecast = displayData.forecast
      ? displayData.forecast.slice(0, 3)
      : [];
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 22 }}>{info.alert}</span>
            <div>
              ...
              <div style={{ fontSize: 11, color: '#888888' }}>
                อัปเดต{' '}
                {displayData.timeLabel.split(' | ')[1] || displayData.timeLabel}
              </div>
            </div>
          </div>
          <strong style={{ fontSize: 20, color: info.color }}>
            {Math.round(displayData.pm25)}{' '}
            <span
              style={{ fontSize: 12, fontWeight: 'normal', color: '#666666' }}
            >
              µg/m³
            </span>
          </strong>
        ...
        {hourlyForecast.length > 0 && (
          ...
          >
            ...
            {hourlyForecast.map((f, i) => (
              ...
              >
                <span style={{ color: '#666666', fontSize: 11 }}>{f.time}</span>
                <div
                  style={{
                    width: 8,
                    height: 8,
                    borderRadius: '50%',
                    background: f.info.color,
                  }}
                ></div>
                <strong style={{ color: '#111111', fontSize: 12 }}>
                  {Math.round(f.pm25)}
                </strong>
              </div>
            ))}
        ...
        {displayData.pm25 > 35 && (
          ...
          >
            🚨 ควรงดการเผาเศษซากพืช/วัสดุเกษตร
          </div>
        )}
        ...
        >
          ...
          >
            <span style={{ fontSize: 11, color: '#666666' }}>PM 10</span>
            <strong style={{ fontSize: 13, color: '#111111' }}>
              {Math.round(displayData.pm10)}{' '}
              <small
          ...
          >
            <span style={{ fontSize: 11, color: '#666666' }}>EU AQI</span>
            <strong style={{ fontSize: 13, color: '#111111' }}>
              {Math.round(displayData.aqi)}{' '}
              <small
                style={{ fontSize: 9, fontWeight: 'normal', color: '#666666' }}
              >
                Index
  ...
  if (isLoading || !coords)
    return (
      <div className="widget-box skeleton-pulse">
        <div className="w-loader">กำลังตรวจสอบพิกัดและคุณภาพอากาศ...</div>
      </div>
    );
  ...
  return (
    <div
      className="widget-box aqi-widget slide-up-anim"
      style={{
        animationDelay: '0.2s',
        display: 'flex',
        flexDirection: 'column',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            >
              📍 จ.{displayData.locationName} &bull; อัปเดต:{' '}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              >
                {Math.round(displayData.pm10)}{' '}
                <small style={{ fontSize: '9px', fontWeight: 'normal' }}>
                  µg/m³
                </small>
              </span>
            </div>
            <div
              style={{ borderLeft: '1px solid #e2e8f0', margin: '0 4px' }}
            ></div>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span
                style={{
