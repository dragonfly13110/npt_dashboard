...
function request(body, headers = {}) {
    return new Request('https://example.netlify.app/.netlify/functions/ai-proxy', {
        method: 'POST',
        headers: {
...
describe('ai-proxy', () => {
    ...
    it('rejects unsupported methods', async () => {
        const response = await handler(new Request('https://example.netlify.app/.netlify/functions/ai-proxy', { method: 'GET' }));
        expect(response.status).toBe(405);
    });

    it('rejects origins outside the allowlist', async () => {
        const response = await handler(request({ provider: 'gemini', body: {} }, { origin: 'https://evil.example' }));
