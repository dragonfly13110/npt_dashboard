...
describe('ai-proxy', () => {
  ...
  it('builds a compact server-owned prompt for landing Gemini chat', async () => {
    ...
    expect(body.systemInstruction.parts[0].text.length).toBeLessThan(6000);
    ...
    for (const route of [
      ...
      expect(body.systemInstruction.parts[0].text).toContain(route);
    }
    expect(fetch).toHaveBeenCalledTimes(2);
  ...
  it('routes STOU research knowledge chat through its dedicated corpus', async () => {
    process.env.GEMINI_API_KEY_1 = 'test-key';
    mockAllowedRateClaim();
    fetch.mockResolvedValueOnce(
      new Response(JSON.stringify({ ok: true }), {
    ...
    const response = await handler(
      request({
        ...
        landing: true,
        knowledgeKind: 'stou',
        knowledgeArticleSlug: 'stou-14305',
        body: {
          model: 'gemini-3.5-flash-lite',
          contents: [
            {
              role: 'user',
              parts: [{ text: 'มีงานวิจัยเรื่องโรคมันสำปะหลังไหม' }],
            },
          ],
          stream: true,
        },
    ...
    expect(JSON.stringify(body.contents)).toContain('/public/stou-research/');
    expect(JSON.stringify(body.contents)).not.toContain('/public/machinery/');
  });

  it('routes frontier agricultural research chat through its dedicated corpus', async () => {
    process.env.GEMINI_API_KEY_1 = 'test-key';
    mockAllowedRateClaim();
    fetch.mockResolvedValueOnce(
      new Response(JSON.stringify({ ok: true }), {
    ...
    const response = await handler(
      request({
        provider: 'gemini',
        landing: true,
        knowledgeKind: 'frontier-agri',
        knowledgeArticleSlug: 'agri-01-001',
        body: {
          model: 'gemini-3.5-flash-lite',
          contents: [
            {
              role: 'user',
              parts: [{ text: 'มีงานวิจัย CRISPR ข้าวทนแล้งไหม' }],
            },
          ],
          stream: true,
        },
