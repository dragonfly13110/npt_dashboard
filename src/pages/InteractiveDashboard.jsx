...
import { areaOption, barOption, pieOption, radarOption, treemapOption } from '../components/charts/echartOptions';
                    ...
                    <Col xs={24} lg={15}>
                        <ChartCard title="📊 เปรียบเทียบรายอำเภอ — กลุ่มเกษตรกร & โครงสร้างพื้นฐาน">
                            <div style={{ height: 300 }} key={animationKey}>
                                <EChart option={barOption(districtGroupBar.map((item) => {
                                    const [, ce, lp, lc, pc, sfc] = Object.values(item);
                                    return { name: item.name, ce, lp, lc, pc, sfc };
                                }), [
                                    { key: 'ce', name: 'วิสาหกิจ', color: (item) => isTargetDistrict(item.name) ? '#10b981' : '#e2e8f0' },
                                    { key: 'lp', name: 'แปลงใหญ่', color: (item) => isTargetDistrict(item.name) ? '#3b82f6' : '#e2e8f0' },
                                    { key: 'lc', name: 'ศพก.', color: (item) => isTargetDistrict(item.name) ? '#f59e0b' : '#e2e8f0' },
                                    { key: 'pc', name: 'ศจช.', color: (item) => isTargetDistrict(item.name) ? '#8b5cf6' : '#e2e8f0' },
                                    { key: 'sfc', name: 'ศดปช.', color: (item) => isTargetDistrict(item.name) ? '#f43f5e' : '#e2e8f0' },
                    ...
                    <Col xs={24} lg={9}>
                        <ChartCard title={`🌾 สัดส่วนพื้นที่เพาะปลูก (${selectedDistrict === 'ทั้งหมด' ? 'ทั้งจังหวัด' : selectedDistrict})`}>
                            <div style={{ height: 240 }} key={animationKey}>
                                <EChart option={pieOption(displayAgriPie, { colors: PIE_COLORS, unit: 'ไร่' })} />
                            </div>
                ...
                <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
                    <Col xs={24} lg={12}>
                        <ChartCard title="🍚 ผลผลิตข้าว — นาปี vs นาปรัง รายอำเภอ">
                            <div style={{ height: 280 }} key={animationKey}>
                                <EChart option={barOption(riceBar.map((item) => {
                                    const [, inSeason, offSeason] = Object.values(item);
                                    return { name: item.name, inSeason, offSeason };
                                }), [
                                    { key: 'inSeason', name: 'ข้าวนาปี', color: (item) => isTargetDistrict(item.name) ? '#16a34a' : '#e2e8f0' },
                                    { key: 'offSeason', name: 'ข้าวนาปรัง', color: (item) => isTargetDistrict(item.name) ? '#86efac' : '#f1f5f9' },
                    ...
                    <Col xs={24} lg={12}>
                        <ChartCard title="🌿 พืชอื่นๆ รายอำเภอ">
                            <div style={{ height: 280 }} key={animationKey}>
                                <EChart option={barOption(cropDistrictBar.map((item) => {
                                    const [, field, fruit, veg, flower, herb] = Object.values(item);
                                    return { name: item.name, field, fruit, veg, flower, herb };
                                }), [
                                    { key: 'field', name: 'พืชไร่', color: (item) => isTargetDistrict(item.name) ? '#f59e0b' : '#e2e8f0' },
                                    { key: 'fruit', name: 'ไม้ผล', color: (item) => isTargetDistrict(item.name) ? '#ef4444' : '#e2e8f0' },
                                    { key: 'veg', name: 'พืชผัก', color: (item) => isTargetDistrict(item.name) ? '#22c55e' : '#e2e8f0' },
                                    { key: 'flower', name: 'ไม้ดอก', color: (item) => isTargetDistrict(item.name) ? '#ec4899' : '#e2e8f0' },
                                    { key: 'herb', name: 'สมุนไพร', color: (item) => isTargetDistrict(item.name) ? '#14b8a6' : '#e2e8f0' },
                                ], { stacked: true, unit: 'ไร่', rotate: 25 })} />
                            </div>
                        </ChartCard>
                    </Col>
                ...
                <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
                    <Col xs={24} sm={12} lg={8}>
                        <ChartCard title="🌾 แปลงใหญ่ — ประเภทสินค้า">
                            <div style={{ height: 220 }} key={animationKey}>
                                <EChart option={pieOption(lpPie, { colors: CHART_COLORS, unit: 'แปลง' })} />
                            </div>
                    ...
                    <Col xs={24} sm={12} lg={8}>
                        <ChartCard title="👥 สถาบันเกษตรกร — ประเภทกลุ่ม">
                            <div style={{ height: 260 }} key={animationKey}>
                                <EChart option={barOption(instituteBar, [{ key: 'value', name: 'จำนวน', color: (item, index) => CHART_COLORS[index % CHART_COLORS.length] }], { layout: 'vertical', unit: 'กลุ่ม', grid: { left: 84 } })} />
                            </div>
                            <div className="chart-footer-note">
                    ...
                    <Col xs={24} lg={8}>
                        <ChartCard title="🤝 วิสาหกิจชุมชน — แยกตามอำเภอ">
                            <div style={{ height: 280 }} key={animationKey}>
                                <EChart option={barOption(ceDistBar.map((item) => {
                                    const [, value] = Object.values(item);
                                    return { name: item.name, value };
                                }), [{ key: 'value', name: 'จำนวน', color: (item, index) => isTargetDistrict(item.name) ? PIE_COLORS[index % PIE_COLORS.length] : '#e2e8f0' }], { layout: 'vertical', unit: 'แห่ง', grid: { left: 78 } })} />
                            </div>
                        </ChartCard>
                    </Col>
                </Row>
                ...
                <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
                    <Col xs={24} lg={selectedDistrict !== 'ทั้งหมด' ? 14 : 24}>
                        <ChartCard title="📈 ครัวเรือนเกษตรกร & พื้นที่เพาะปลูก รายอำเภอ">
                            <div style={{ height: 280 }} key={animationKey}>
                                <EChart option={areaOption(householdsArea.map((item) => {
                                    const [, households, area] = Object.values(item);
                                    return { name: item.name, households, area };
                                }), [
                                    { key: 'households', name: 'ครัวเรือน', color: '#10b981' },
                    ...
                    {selectedDistrict !== 'ทั้งหมด' && (
                        <Col xs={24} lg={10}>
                            <ChartCard title={`🎯 Radar — ${selectedDistrict}`}>
                                <div style={{ height: 280 }} key={animationKey}>
                                    <EChart option={radarOption(radarData, { name: selectedDistrict, labelKey: 'subject', valueKey: 'pct', max: 100, color: '#10b981' })} />
                                </div>
                            </ChartCard>
                        </Col>
                    )}
                ...
                <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
                    <Col xs={24} lg={14}>
                        <ChartCard title="🗂️ ภาพรวมข้อมูลในระบบ — Treemap ตามกลุ่มงาน">
                            <div style={{ height: 280 }} key={animationKey}>
                                <EChart option={treemapOption(flatTreemap, { colors: TREEMAP_COLORS, valueKey: 'size', unit: 'รายการ' })} />
                            </div>
                        </ChartCard>
                    </Col>
                    <Col xs={24} lg={10}>
