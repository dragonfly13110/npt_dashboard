...
const columns = [
  {
    ...
    key: 'acq_date',
    width: 110,
    sorter: (a, b) => new Date(a.acq_date) - new Date(b.acq_date),
  },
  {
    title: 'เวลา',
    dataIndex: 'acq_time',
    key: 'acq_time',
    width: 80,
    render: (v) => (v ? `${v.substring(0, 2)}:${v.substring(2)} น.` : '-'),
  },
  {
    ...
    render: (v) => {
      ...
      return conf === 'high' ? (
        <span style={{ color: '#cf222e', fontWeight: 600 }}>สูง (High)</span>
      ) : conf === 'nominal' ? (
        <span style={{ color: '#bf8700', fontWeight: 600 }}>ปานกลาง (Nom)</span>
      ) : conf === 'low' ? (
        <span style={{ color: '#1a7f37' }}>ต่ำ (Low)</span>
      ) : (
        '-'
      );
    },
  ...
  <>
    <Row gutter={16}>
      <Col span={12}>
        <Form.Item
          name="acq_date"
          label="วันที่พบ (YYYY-MM-DD)"
          rules={[{ required: true }]}
        >
          <Input placeholder="2026-04-20" />
        </Form.Item>
      </Col>
      <Col span={12}>
        <Form.Item name="acq_time" label="เวลา (HHMM)">
          <Input placeholder="1350" />
        </Form.Item>
      </Col>
    </Row>
    <Row gutter={16}>
    ...
    <Row gutter={16}>
      <Col span={12}>
        <Form.Item name="confidence" label="ระดับความน่าเชื่อถือ">
          <Select
            options={['High', 'Nominal', 'Low'].map((d) => ({
              label: d,
              value: d.toLowerCase(),
            }))}
          />
      ...
      <Col span={12}>
        <Form.Item name="frp" label="ค่าพลังงานความร้อน (FRP)">
    ...
    <Row gutter={16}>
      <Col span={12}>
        <Form.Item name="land_use" label="ประเภทพื้นที่">
          <Input placeholder="เช่น พื้นที่เกษตร" />
        </Form.Item>
      </Col>
      <Col span={12}>
        <Form.Item name="village" label="หมู่บ้าน/ชุมชน">
          <Input />
        </Form.Item>
      </Col>
    </Row>
    <Row gutter={16}>
      <Col span={12}>
        <Form.Item name="satellite" label="ดาวเทียม">
          <Input placeholder="VIIRS / Suomi NPP" />
        </Form.Item>
      </Col>
      <Col span={12}>
        <Form.Item name="year" label="ปีที่บันทึก">
          <InputNumber style={{ width: '100%' }} />
        </Form.Item>
      </Col>
    </Row>
...
const confidences = [
  ...
  { label: 'ปานกลาง (Nominal)', value: 'nominal' },
...
export default function FireHotspots() {
  ...
  const handleSyncHotspots = async (refetch) => {
    setSyncing(true);
    try {
      const res = await fetch('/.netlify/functions/sync-hotspots', {
        method: 'POST',
      });
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(
          getSyncErrorMessage(res.status, payload.error || payload.message)
        );
      }

      const latest = payload.latest?.acq_date
        ? ` ล่าสุด ${payload.latest.acq_date}`
        : '';
      message.success(
        `${payload.message || 'ดึงข้อมูล GISTDA สำเร็จ'}${latest}`
      );
      refetch?.();
    } catch (err) {
