...
const columns = [
  ...
  {
    title: 'ปีที่จัดตั้ง_พ.ศ.',
    dataIndex: 'established_year_th',
    key: 'established_year_th',
    width: 100,
  },
  {
    title: 'ปีที่จัดตั้ง_ค.ศ.',
    dataIndex: 'established_year_en',
    key: 'established_year_en',
    width: 100,
  },
  {
    title: 'ระดับชั้น',
...
const formFields = (
  <>
    <Form.Item
      name="center_name"
      label="ชื่อ_ศจช (ชื่อศูนย์)"
      rules={[{ required: true }]}
    >
      <Input />
    </Form.Item>
    <Form.Item name="established_year_th" label="ปีที่จัดตั้ง_พ.ศ.">
      <Input />
    </Form.Item>
    <Form.Item name="established_year_en" label="ปีที่จัดตั้ง_ค.ศ.">
      <Input />
    </Form.Item>
    <Form.Item name="grade_level" label="ระดับชั้น">
      <Input placeholder="เช่น A, B, C" />
    </Form.Item>
    <Form.Item name="member_count" label="สมาชิก_ราย">
    ...
    <Form.Item name="main_crop_type" label="ชนิดพืชหลัก">
      <Input />
    </Form.Item>
    <Form.Item name="location_type" label="พื้นที่ตั้ง">
      <Input placeholder="เช่น พื้นที่ปกติ/ทั่วไป" />
    </Form.Item>
    <Form.Item name="chairman" label="ชื่อ_สกุล_ประธาน">
      <Input />
    </Form.Item>
    <Form.Item name="other_status" label="สถานะอื่นๆ">
      <Input placeholder="เช่น ศดปช." />
    </Form.Item>
    <Form.Item name="contact_phone" label="เบอร์ติดต่อ">
      <Input />
    </Form.Item>
    <Form.Item name="latitude" label="ละติจูด (Latitude)">
      <InputNumber
        placeholder="เช่น 13.85455"
        style={{ width: '100%' }}
        precision={6}
      />
    </Form.Item>
...
export default function PestCenters() {
  ...
  const fetchPestCenters = async () => {
    const { data, error } = await supabase.from('pest_centers').select('*');
  ...
  const gradeOptions = useMemo(() => {
    const unique = [
      ...new Set(chartData.map((d) => d.grade_level).filter(Boolean)),
    ].sort();
    return unique.map((d) => ({ label: d, value: d }));
  }, [chartData]);
  ...
  const cropOptions = useMemo(() => {
    const unique = [
      ...new Set(chartData.map((d) => d.main_crop_type).filter(Boolean)),
    ].sort();
    return unique.map((d) => ({ label: d, value: d }));
  }, [chartData]);
  ...
  const filteredData = useMemo(() => {
    return chartData.filter((item) => {
      ...
      if (filterGradeLevel && item.grade_level !== filterGradeLevel)
        return false;
      if (filterCropType && item.main_crop_type !== filterCropType)
        return false;
      return true;
    });
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: 16, fontWeight: 700, color: '#1f2328' }}>
            สรุปข้อมูลศูนย์จัดการศัตรูพืชชุมชน (ศจช.)
          </span>
          <Tag color="green">
            {hasActiveFilter
        ...
        {chartLoading ? (
          ...
          <Row gutter={[24, 24]}>
            ...
            <Col xs={24} lg={12}>
              <Card
                title="สรุปตามอำเภอ (แยกตามระดับชั้น)"
                size="small"
                bordered={false}
                style={{ background: '#fafbfc' }}
              >
