...
function FrontierAgriResearchArticle({ catalog, config }) {
  ...
  useEffect(() => {
    ...
    setError(false);
    fetch(
      `${config.catalogUrl.replace(/\/catalog\.json$/, '')}/articles/${article.article_file}`
    )
      .then((response) => {
        if (!response.ok)
      ...
      .catch((loadError) => {
        console.error(
          'Error loading agricultural research article:',
          loadError
        );
        setError(true);
      })
    ...
    <main className="rice-page stou-page rice-article frontier-agri-page">
      ...
      <div className="rice-article-layout">
        ...
        <aside className="rice-article-aside">
          ...
          <div className="rice-aside-note">{config.articleNote}</div>
        </aside>
      </div>
    </main>
  );
