...
const columns = [
  {
    ...
    key: 'code',
    width: 80,
    sorter: (a, b) => (Number(a.code) || 0) - (Number(b.code) || 0),
  },
  {
    title: 'ปี',
    dataIndex: 'year',
  ...
  },
  {
    ...
    width: 120,
    sorter: (a, b) =>
      String(a.commodity_group || '').localeCompare(
        String(b.commodity_group || ''),
        'th'
      ),
  },
  {
    ...
    width: 110,
    sorter: (a, b) =>
      String(a.commodity || '').localeCompare(String(b.commodity || ''), 'th'),
  },
  {
    title: 'สินค้ารอง',
    dataIndex: 'secondary_commodity',
    key: 'secondary_commodity',
    width: 110,
    sorter: (a, b) =>
      String(a.secondary_commodity || '').localeCompare(
        String(b.secondary_commodity || ''),
        'th'
      ),
  },
  {
    ...
    width: 260,
    sorter: (a, b) =>
      String(a.plot_name || '').localeCompare(String(b.plot_name || ''), 'th'),
  },
  {
    title: 'อำเภอ',
    dataIndex: 'district',
  ...
  },
  {
    ...
    width: 120,
    sorter: (a, b) =>
      String(a.agency || '').localeCompare(String(b.agency || ''), 'th'),
  },
];
...
const formFields = (
  <>
    ...
    <Form.Item name="year" label="แปลงใหญ่ปี">
      <InputNumber style={{ width: '100%' }} placeholder="2568" />
    </Form.Item>
    <Form.Item name="commodity_group" label="กลุ่มสินค้า">
      <Input placeholder="ข้าว, ผัก/สมุนไพร" />
    </Form.Item>
    <Form.Item name="commodity" label="สินค้าหลัก" rules={[{ required: true }]}>
      <Input placeholder="ข้าว" />
    </Form.Item>
    <Form.Item name="secondary_commodity" label="สินค้ารอง">
      <Input placeholder="ข้าวโพดฝักอ่อน" />
    </Form.Item>
    <Form.Item
      name="plot_name"
      label="ชื่อแปลงใหญ่"
    ...
    <Form.Item name="member_count" label="จำนวนเกษตรกร">
      <InputNumber style={{ width: '100%' }} />
    </Form.Item>
    <Form.Item name="area_rai" label="พื้นที่ (ไร่)">
      <InputNumber style={{ width: '100%' }} step={0.01} />
    </Form.Item>
    <Form.Item name="agency" label="หน่วยงาน">
      <Input placeholder="กรมการข้าว, กรมส่งเสริมการเกษตร" />
    </Form.Item>
...
const columnTitle = (key) =>
  columns.find((c) => c.dataIndex === key)?.title || key;

const subdistrictOptionsFor = (district) => {
  const normalizedDistrict = district ? normalizePlaceName(district) : null;
  const values = (nptSubdistrictsGeoJSON.features || [])
    .filter(
      (feature) =>
        !normalizedDistrict ||
...
export default function LargePlots() {
  ...
  useEffect(() => {
    ...
    if (meta)
      meta.setAttribute(
        'content',
        'ข้อมูลแปลงใหญ่จังหวัดนครปฐม พร้อมแผนภูมิ สถิติ และตารางค้นหาข้อมูลตามอำเภอและปี'
      );
  }, []);
  ...
  const yearOptions = useMemo(() => {
    const unique = [
      ...new Set(dashboardData.map((d) => d.year).filter(Boolean)),
    ].sort();
  ...
  const districtOptions = useMemo(() => {
    const unique = [
      ...new Set(dashboardData.map((d) => d.district).filter(Boolean)),
    ].sort();
  ...
  const dashboardFilterConfig = useMemo(() => {
    const subdistrictFilter = {
      key: 'subdistrict',
      label: columnTitle('subdistrict'),
      options: subdistrictOptionsFor(pageFilters.district),
    };
    return filterConfig.flatMap((filter) =>
      filter.key === 'district' ? [filter, subdistrictFilter] : [filter]
    );
  }, [pageFilters.district]);
  ...
  const filteredData = useMemo(() => {
    return dashboardData.filter((item) => {
      return Object.entries(pageFilters).every(
        ([key, value]) =>
          value === undefined ||
          value === null ||
          value === '' ||
  ...
  const fetchTableData = ({ pagination, search, searchField, sorter }) => {
    ...
    <div>
      ...
      >
        ...
        >
          ...
          {Object.values(pageFilters).some(
        ...
        {loading ? (
          ...
          <Row gutter={[24, 24]}>
            <Col xs={24} lg={12}>
              <Card
                title="ภาพรวมสัดส่วนแต่ละกลุ่มสินค้า"
                size="small"
                bordered={false}
            ...
            <Col xs={24} lg={12}>
              <Card
                title={
                  pageFilters.district
                    ? `จำนวนแปลงใหญ่ใน ${pageFilters.district}`
                    : 'จำนวนแปลงใหญ่แยกตามอำเภอ'
                }
                size="small"
                bordered={false}
                style={{ background: '#fafbfc' }}
