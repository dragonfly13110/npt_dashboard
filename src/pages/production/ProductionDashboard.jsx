          ...
          >
            ...
            <CategoryBentoCard
              ...
              mainStats={[
                ...
                {
                  label: 'สมาชิก (ราย)',
                  value: lpStats.members.toLocaleString(),
                  isTotal: true,
                },
                {
                  label: 'พื้นที่ (ไร่)',
                  value: lpStats.area.toLocaleString(),
                  isTotal: true,
                },
              ]}
            />
            ...
            <CategoryBentoCard
              ...
              mainStats={[
                ...certStats.topCrops.map(([c, count]) => ({
                  label: c,
                  value: count,
                })),
                {
                  label: 'พื้นที่รับรองรวม (ไร่)',
                  value: certStats.area.toLocaleString(),
                  isTotal: true,
                },
              ]}
            />
                  ...
                  <EChart
                    option={barOption(
                      ...
                      lpGroups.map((group, index) => ({
                        key: group,
                        color: LP_COLORS[index % LP_COLORS.length],
                      })),
                      {
                        stacked: true,
                        colors: LP_COLORS,
                ...
                {certBar.length > 0 ? (
                  <EChart
                    option={barOption(
                      ...
                      { stacked: true, colors: CERT_COLORS, unit: 'รายการ' }
                    )}
                  />
                ) : (
                  <EmptyChart label="มาตรฐาน GAP" />
              ...
              <CategoryChartCard title="✅ ปริมาณผลผลิตรวม GAP (กิโลกรัม) - 10 อันดับแรก">
                {certVolumeData.length > 0 ? (
                  <EChart
                    option={barOption(
                      certVolumeData,
                      [{ key: 'value', name: 'ผลผลิต' }],
                      {
                        layout: 'vertical',
                        colors: CERT_COLORS.slice(4).concat(CERT_COLORS),
                        unit: 'กก.',
                        grid: { left: 76 },
                      }
                    )}
              ...
              <Col xs={24} lg={12}>
                <CategoryChartCard title="📊 ผลผลิตพืชรายอำเภอ">
                  <EChart
                    option={barOption(
                      cropBar,
                      [{ key: 'value', name: 'จำนวน', color: '#ab47bc' }],
                      { colors: ['#ab47bc'] }
                    )}
                  />
                </CategoryChartCard>
