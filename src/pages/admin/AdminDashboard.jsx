...
function miniBarOption(data) {
  const option = barOption(
    data,
    [{ key: 'value', name: 'จำนวน', color: '#1a7f37' }],
    {
  ...
  return {
    ...
    yAxis: {
      ...option.yAxis,
      ...axisBase,
      splitLine: { lineStyle: { color: '#eef1f4', type: 'dashed' } },
    },
  };
}
...
export default function AdminDashboard() {
  ...
  const pieData = stats
    .filter((s) => s.count > 0)
    .map((s) => ({ name: s.label, value: s.count }));
  const topCategory = stats.reduce(
    (best, item) => (item.count > (best?.count ?? -1) ? item : best),
    null
  );
  const latestActivity = activities[0]?.created_at
    ...
    <div className="admin-overview-page">
      <section className="admin-overview-hero">
        <div>
          ...
          <p>
            ติดตามข้อมูลบุคลากร พัสดุ/ครุภัณฑ์ งบประมาณ
            และความเคลื่อนไหวล่าสุดจากฐานข้อมูลกลาง
          </p>
        </div>
        <div
          className="admin-overview-hero-metrics"
          aria-label="สรุปภาพรวมฝ่ายบริหาร"
        >
          <div>
      ...
      <section className="admin-overview-stat-grid" aria-label="ตัวชี้วัดหลัก">
        {loading
          ? [1, 2, 3].map((i) => (
              <div
                key={i}
                className="admin-overview-card admin-overview-stat-card"
              >
          ...
          : stats.map((s) => {
              ...
              return (
                <article
                  key={s.table}
                  className={`admin-overview-card admin-overview-stat-card is-${s.color}`}
                >
                  <div className="admin-overview-stat-top">
                    <div className="admin-overview-stat-icon">
                      <IconComp />
                    </div>
                    <div className="admin-overview-share">
                      {getShare(s.count, totalRecords)}%
                    </div>
                  </div>
                  ...
                  <div className="admin-overview-stat-note">
                    รวม{s.shortLabel}ในระบบบริหาร
                  </div>
                </article>
              );
            })}
        ...
        <article className="admin-overview-card admin-overview-chart-card">
          ...
          <div className="admin-overview-chart">
            {barData.length > 0 ? (
              ...
              <div className="admin-overview-empty-inline">
        ...
        <article className="admin-overview-card admin-overview-chart-card">
          <div className="admin-overview-card-header">
            <div>
              <h2>สัดส่วนข้อมูล</h2>
              <p>
                เห็นน้ำหนักของแต่ละหมวดเมื่อเทียบกับข้อมูลทั้งหมด{' '}
        ...
        <article className="admin-overview-card admin-overview-chart-card">
          ...
          <div className="admin-overview-chart">
            {trendLoading ? (
              ...
              <EChart
                option={miniLineOption(trendData)}
                style={{ height: 280 }}
              />
            )}
          </div>
            ...
            <ul className="admin-overview-activity-list">
              {activities.map((act, i) => {
                const matchedTable = tables.find(
                  (table) => table.label === act.table
                );
                const IconComp =
                  matchedTable?.iconComponent || DatabaseOutlined;
                return (
                  <li key={i} className="admin-overview-activity-item">
                    <div
                      className={`admin-overview-activity-icon is-${matchedTable?.color || 'green'}`}
                    >
                      <IconComp />
                    </div>
