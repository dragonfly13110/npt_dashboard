...
const allTables = groupConfig.flatMap((g) =>
  g.tables.map((t) => ({
    ...t,
    group: g.group,
    groupIcon: g.icon,
    ...
    },
    {
      ...
      render: () => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontWeight: 600 }}>เพิ่ม/อัปเดตข้อมูล</span>
        </div>
      ),
    },
    {
      ...
      render: (count, record) => (
        ...
        >
          <Badge
            ...
            style={{
              backgroundColor:
                count >= 100 ? '#f44336' : count >= 10 ? '#ff9800' : '#43a047',
              fontSize: 13,
              fontWeight: 600,
              padding: '0 12px',
              cursor: 'pointer',
    ...
    <div className="crud-container">
      <div className="md-page-header">
        ...
        <p>
          ...
          (ข้อมูลที่ถูกเพิ่มในช่วงเวลาใกล้เคียงกัน ±5 นาที
          จะถูกรวมเป็นครั้งเดียว)
        </p>
      </div>
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total, range) =>
            `${range[0]}-${range[1]} จาก ${total} ครั้ง`,
        }}
      />
    </div>
  );
