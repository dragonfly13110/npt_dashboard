...
async function fetchDatabaseContext(query, modelKey, chatHistory = []) {
    ...
    for (const table of matchedTables) {
        try {
            ...
            // NO LIMITS: Fetch maximum possible rows to ensure AI has 100% full context
            ...
            // ── Build accurate district summary from ALL data (not limited sample) ──
            // This prevents AI from miscounting when sample data is incomplete
            ...
            try {
                ...
                // Fetch up to 10000 rows for district counting only (lightweight - single column)
...
function buildContextForAI(analysis) {
    ...
    return JSON.stringify(results.map(r => {
        ...
        // Advanced Auto-Aggregation Builder makes the AI a genius at precise numbers
        // It calculates grouped counts for ALL categorical columns so the LLM doesn't have to guess or manually count tokens!
        ...
        if (r.sample && r.sample.length > 0) {
            ...
            keysToAnalyze.forEach(key => {
                ...
                // If it's a categorical field (e.g. types, status, district) having between 1 and 60 unique values
