...
function YoungFarmerGroupsMap({ rows, year }) {
  ...
  return (
    ...
    >
            ...
            <Popup>
              <div style={{ minWidth: 220, fontFamily: 'inherit' }}>
                ...
                >
                  <span>
                    สมาชิก:{' '}
                    <strong>{number.format(item.member_count || 0)}</strong> ราย
                  </span>
                  <span>
                  ...
                  <span>
                    ทุน:{' '}
                    <strong>{number.format(item.fund_management || 0)}</strong>{' '}
                    บาท
                  </span>
                  <span>
                    รายได้: <strong>{number.format(item.income || 0)}</strong>{' '}
                    บาท
                  </span>
                </div>
              </div>
      ...
      >
        <span>
          <EnvironmentOutlined style={{ color: '#1a7f37' }} /> ศักยภาพดี
        </span>
        <span>
          <EnvironmentOutlined style={{ color: '#0969da' }} /> จุดพิกัดกลุ่ม
        </span>
      </div>
    </Card>
  );
...
export default function YoungFarmerGroupsDashboard() {
  ...
  const fetchRows = async () => {
    const { data, error } = await supabase
      ...
      .select(
        getPublicSelectColumns('young_farmer_groups_detailed', columns, role)
      )
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useApiCache(['young_farmer_groups_detailed_all', role], fetchRows);
  const years = useMemo(
    () =>
  ...
  const filteredRows = useMemo(() => {
    ...
    return yearRows.filter((row) => {
      ...
      if (
        ...
        return false;
      if (filters.model_group && row.model_group !== filters.model_group)
        return false;
      if (
  ...
  const modelGroupOptions = useMemo(
    () => makeOptions(yearRows, 'model_group'),
    [yearRows]
  );
  ...
  const baseVisibleColumns = useMemo(
    () =>
      getPublicColumns('young_farmer_groups_detailed', columns, role)
        .filter(
          (column) =>
    ...
    <div>
              ...
              <div style={{ height: 300 }}>
                <EChart
                  option={barOption(
                    ...
                    { layout: 'vertical', unit: 'ราย', grid: { left: 108 } }
                  )}
                />
              </div>
            </Card>
      ...
      <div className="crud-container">
        <div className="crud-header">
          <div className="crud-header-left">
            ...
            <span className="crud-title">
              ตารางข้อมูลกลุ่มยุวเกษตรกร ปี {activeYear || '-'}
            </span>
        ...
        >
          ...
          <Select
            allowClear
            placeholder="กลุ่มต้นแบบ"
            value={filters.model_group}
            onChange={(value) => setFilter('model_group', value)}
      ...
      <Modal
        title={
          editingRecord
            ? 'แก้ไขข้อมูลกลุ่มยุวเกษตรกร'
            : 'เพิ่มข้อมูลกลุ่มยุวเกษตรกร'
        }
        open={editOpen}
        onCancel={() => {
          setEditOpen(false);
