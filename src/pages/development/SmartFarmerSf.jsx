...
const columns = [
    ...
    { title: 'เลขประจำตัวประชาชน', dataIndex: 'citizen_id', key: 'citizen_id', width: 160, importHeader: 'เลขประจำตัวประชาชน' },
    ...
    { title: 'ช่องทางจำหน่าย', dataIndex: 'sales_channel', key: 'sales_channel', width: 180, importHeader: 'ช่องทางการจำหน่ายผลผลิต/สินค้า' },
    {
        ...
        width: 140,
        align: 'right',
        importHeader: 'รายได้รวมภาคการเกษตรของครัวเรือนเกษตรกรปีที่ผ่านมา',
...
export default function SmartFarmerSf() {
    ...
    const exportRows = (format) => {
        const headers = columns.filter((column) => column.dataIndex).map((column) => column.title);
        const keys = columns.filter((column) => column.dataIndex).map((column) => column.dataIndex);
                    ...
                    <div className="crud-header-right">
                        <Popover content={columnSelector} trigger="click" placement="bottomRight">
                            <Button icon={<SettingOutlined />}>
                                คอลัมน์ {visibleColumns.length}/{columns.length}
                            </Button>
                        </Popover>
                    </div>
                </div>
