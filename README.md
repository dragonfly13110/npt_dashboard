...
| Export/Canvas/PDF    | CSV utilities, html2canvas, jsPDF                               |
...
| คำสั่ง                   | ความหมาย              |
| ------------------------ | --------------------- |
| `pnpm run dev`           | เปิด Vite dev server  |
| `pnpm run build`         | build production      |
| `pnpm run build:netlify` | build สำหรับ Netlify  |
| `pnpm run preview`       | preview ไฟล์ build    |
| `pnpm run lint`          | ตรวจ ESLint           |
| `pnpm run test`          | รัน Vitest            |
| `pnpm run test:watch`    | รัน Vitest watch mode |
| `pnpm run test:e2e`      | รัน Playwright e2e    |
| `pnpm run test:e2e:ui`   | เปิด Playwright UI    |
| `pnpm run prerender`     | รัน script prerender  |
...
3. ใช้ build command `pnpm run build:netlify`
...
## การแก้ไขปัญหาและเพิ่มประสิทธิภาพระบบ (Troubleshooting)

### ปัญหาพื้นที่จัดเก็บไม่เพียงพอในสภาพแวดล้อม Build (ENOSPC)

หากนำโปรเจกต์ไปรันบน CI/CD หรือ Cloud build (เช่น Netlify) ที่จำกัดขนาดดิสก์ และพบปัญหาระบบ build พังเนื่องจากไม่มีพื้นที่เหลือ แนะนำให้ดำเนินการดังนี้:

1. **ใช้ pnpm แทน npm:** pnpm ใช้กลไก Hard Links และ Global Store ทำให้ลดการเขียนไฟล์ซ้ำในโปรเจกต์และประหยัดพื้นที่กว่า 50%
2. **ทำความสะอาด cache ในโปรเจกต์:**
   ```bash
   # ล้าง npm cache เดิมที่อาจค้างหรือเสียหาย
   npm cache clean --force
   # ล้างโฟลเดอร์ temp/cache ของโปรเจกต์
   rm -rf tmp/vite-cache/
   # ล้าง pnpm store เพื่อลบ package ที่ไม่ใช้งาน
   pnpm store prune
   ```
