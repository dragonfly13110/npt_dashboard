...
if (!SUPABASE_URL || !SUPABASE_KEY || !GISTDA_KEY) {
  throw new Error(
    'Missing required env for sync-hotspots. Set VITE_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY or VITE_SUPABASE_ANON_KEY, and GISTDA_API_KEY.'
  );
}
...
const syncHandler = async (_event, context) => {
  ...
  try {
    ...
    if (features.length === 0) {
      return {
        statusCode: 200,
        body: JSON.stringify({ message: 'No data found from GISTDA' }),
      };
    }
    ...
    console.log(
      `Success: Sync'd ${rows.length} records. Latest: ${latestRecord?.acq_date || '-'} ${latestRecord?.acq_time || ''}`
    );
    return {
      statusCode: 200,
      body: JSON.stringify({
