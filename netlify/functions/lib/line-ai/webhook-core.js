...
import { createLineAiStore } from './store.js';
import { createKeyPool } from './key-pool.js';
import { createGeminiClient } from './gemini.js';
import { executeTools } from './tools.js';
import { renderAiReply } from './flex.js';
import { createLineAiOrchestrator } from './orchestrator.js';
...
export function setLineAiOrchestrator(orchestrator) {
...
export async function handlePostbackEvent(event) {
  const replyToken = event.replyToken;
  ...
  if (params.action === 'personnel_summary') {
    ...
    const rows = (data || []).filter((row) => row.position !== 'สำนักงาน');
