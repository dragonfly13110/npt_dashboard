...
- ศูนย์องค์ความรู้ /public/knowledge-hub, คลังสารป้องกันศัตรูพืช /public/pesticides, องค์ความรู้การผลิตกล้วยไม้ /public/orchids, น้องข้าวหลาม ทบก. /public/farmer-manual, คลังงานวิจัย มสธ. /public/stou-research, คลังบทความด้านการเกษตรทั่วโลก /public/frontier-agri-research, คลังงานวิจัยพืชนครปฐม /public/npt-research, หลักการเพาะปลูกพืชที่สำคัญ /public/plant-cultivation, คำอธิบายข้อมูล /public/data-dictionary
    ...
    const questions =
      provider === 'gemini'
        ? (Array.isArray(body.contents) ? body.contents : [])
            ...
            .map(textFromGeminiContent)
        : (Array.isArray(body.messages) ? body.messages : [])
            .filter((item) => item?.role === 'user')
            .map((item) => String(item.content || '').trim());
    const hasQuestion = questions.some(
      (question) => question.length > 0 && question.length <= 1000
    );
...
export default async (req, context) => {
  ...
  try {
    ...
    if (validation.landing && validation.pesticideBot) {
